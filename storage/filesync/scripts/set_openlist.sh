...
install_openlist_env()
{
	...
	local name="${openlist_config[name]}"
	...
	if [ "$arg" = "init" ]; then
		if [ -z "$(find "$install_dir" -maxdepth 1 -type f -name "${name}*" -print -quit 2>/dev/null)" ]; then
			...
			if ! findpath=$(find_latest_archive "$downloads_dir" ".*${name}.*"); then
				...
				# 下载文件并验证
				local download_file
				download_file=$(download_openlist "$downloads_dir") && [ -n "$download_file" ] || {
		...
		if [[ ! -d "${openlist_config[sys_path]}" && ! -e "${openlist_config[bin_file]}" ]]; then
			# 安装二进制文件
			install_binary "$target_path" "${openlist_config[bin_file]}" "/usr/local/bin/$name" || return 4
